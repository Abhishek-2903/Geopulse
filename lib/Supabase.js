...
export const dbHelpers = {
  ...
  async createPaymentRecord(userId, amount, currency, razorpayOrderId, downloadCount = 25) {
    try {
      ...
      const { data, error } = await supabase
        ...
        .upsert({
          ...
          status: 'pending',
          downloads_purchased: downloadCount,
      ...
      if (error) {
        console.error('Payment record creation error details:', error);
        throw error;
      }
      return data;
    } catch (error) {
  ...
  async updatePaymentStatus(razorpayOrderId, status, razorpayPaymentId = null) {
    try {
      ...
      if (error) {
        console.error('Payment status update error details:', error);
        throw error;
      }
      return data;
    } catch (error) {
  ...
  // Server-side payment record creation (bypasses RLS using service role)
  createPaymentRecordServer: async (supabaseServerClient, userId, amount, currency, orderId, downloadCount) => {
    try {
      ...
      console.error('Database error in createPaymentRecordServer:', error);
      throw error;
    }
  },

  // Server-side payment status update (bypasses RLS using service role)
  updatePaymentStatusServer: async (supabaseServerClient, orderId, status, paymentId = null) => {
    try {
      const updateData = {
        status: status,
        updated_at: new Date().toISOString()
      ...
      console.error('Database error in updatePaymentStatusServer:', error);
      throw error;
    }
  },
};